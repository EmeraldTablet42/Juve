      ...
      <div className="right">
        <div className="info-line">
          {!auth.isLogined ? (
            <>
              <div>
                <button><Link to="/member/join">회원가입</Link></button>
              </div>
              <div>
          ...
          {auth.memberId === "adminjuve" ? (
            <div>
              <button><Link to="/admin/main">관리자페이지</Link></button>
            </div>
          ) : (
            <>
              <div>
                <button><Link to="/member/mypage/myorder">마이페이지</Link></button>
              </div>
              <div>
                <button><Link to="/cart"><img src={sampleimage} alt="장바구니" style={{width:"40px",height:"40px"}}/></Link></button>
              </div>
            </>
          )}
        </div>
