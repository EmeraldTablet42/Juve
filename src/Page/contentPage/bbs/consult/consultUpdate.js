...
const ConsultUpdate = () => {
  ...
  const getBoard = () => {
    axios
      ...
      .then((res) => {
        ...
        alert(JSON.stringify(res.data.context));
  ...
  const regConsult = () => {
    ...
    alert(bbsDB.date);
    fd.set("date", new Date(bbsDB.date));
    fd.set("photos", imgUploaded);
    fd.set("hits", bbsDB.hits);
  ...
  return (
    <>
      ...
      />
      <button onClick={regConsult}>등록</button>
      <button>취소</button>
      <button
        onClick={() => {
